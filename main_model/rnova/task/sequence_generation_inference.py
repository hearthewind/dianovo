...
def sequence_generation_inference(cfg: DictConfig, spec_header, test_dl, model, model_gnova, device):
    ...
    for d in tqdm(test_dl, total=len(test_dl)):
        ...
        # raw_peak_number = get_total_peak_num(spec_head['MS1 Peak Number'], spec_head['MS2 Peak Number'], spec_head['Charge'], cfg)
        ...
        with torch.no_grad():
            with autocast(dtype=torch.bfloat16):
                gnova_encoder_outputs = model_gnova.encoder(**gnova_encoder_input_list[0])
                gnova_logits = model_gnova.iontype_finear(gnova_encoder_outputs[-1])
