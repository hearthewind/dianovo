...
class OptimalPathInference:
    ...
    def extract_totmass_from_graphprob_input(self, graph_probability_input, encoder_input):
        ...
        if current_mass > 0.0 or (current_mass == 0.0 and seq_len == 1):
            ...
            raise ValueError("last_graph_label points to a negative moverz")
    ...
    def path_comparison(self, path1, path2):
        # path1 and path2 are the result of extract_path_from_graphprob_input
        path1_len = len(path1)
        path2_len = len(path2)

        path1_mass_cum = np.cumsum(path1)
        path2_mass_cum = np.cumsum(path2)

        num_match = 0
        i = 0
        j = 0
        while i < path1_len and j < path2_len:
            if abs(path1_mass_cum[i] - path2_mass_cum[j]) < 0.2:
                if abs(path1[i] - path2[j]) < 0.02:
                    num_match += 1
                i += 1
                j += 1
            elif path1_mass_cum[i] < path2_mass_cum[j]:
                i += 1
            else:
                j += 1

        return num_match == path1_len == path2_len
    ...
    def beam_best_k(self, graph_probability_input_list, score_list, graph_probability_input_complete, score_complete,
        ...
        _, sorted_index = torch.sort(score_list_extend_concat, descending=True)
        topk_index = sorted_index[:(beam_size*3)]
        ...
        for idx in topk_index:
            ...
            if graph_index < len(graph_probability_list):
                ...
                new_path = self.extract_path_from_graphprob_input(graph_probability_input, encoder_input)

                found = False
                for exist_path in existing_paths:
                    if self.path_comparison(new_path, exist_path):
                        found = True
                        break
                if found:
                    continue
            ...
            if len(topk_graph_probability_input) >= beam_size:
                break
    ...
    def inference(self):
        ...
        for encoder_input, _, _, label, label_mask, idx in tqdm(self.test_dl, total=len(self.test_dl)):
            ...
            with torch.no_grad():
                ...
                matched_num, predict_len, label_len, pred_path, label_path = self.path_evaluation(graph_probability_input, label, encoder_input)
