...
def main(worker, file_worker, total_worker, total_file_worker, input_mzml_folder, input_feature_file, input_whole_feature_file, output_folder)->None:
    ...
    for psm_head, mzml_file in zip(final_psm_list, chosen_mzml_list):
        ...
        reader = open(os.path.join(output_folder, f'{mzml_file}_{file_worker}_{file_num}.msgp'), 'rb')
        writer = open(os.path.join(output_folder, f'{mzml_file}_{file_worker}_{file_num}.msgp'), 'ab')

        msgp_existing_size = os.stat(os.path.join(output_folder, f'{mzml_file}_{file_worker}_{file_num}.msgp'))
        file_size += msgp_existing_size.st_size

        with open(os.path.join(output_folder, f'{mzml_file}_{file_worker}.csv'), 'r') as index_reader:
            existing_lines = index_reader.readlines()
        existing_header = pd.read_csv(os.path.join(output_folder, f'{mzml_file}_{file_worker}.csv'), index_col='Spec Index')

        with open(os.path.join(output_folder, f'{mzml_file}_{file_worker}.csv'), 'a', buffering=1) as index_writer:
            ...
            for peptide in tqdm(psm_head):
                ...
                if spec_index in existing_header.index:
                    if spec_index == existing_header.index[-1]:
                        last_head = existing_header.iloc[-1]
                        try:
                            reader.seek(last_head['MSGP Datablock Pointer'])
                            pickle.loads(gzip.decompress(reader.read(last_head['MSGP Datablock Length'])))
                            continue
                        except Exception as e:
                            print('Error when reading existing MSGP: ', spec_index)
                            traceback.print_exc()
                    else:
                        continue
