...
def create_interpolated_ms1(precursor: pd.Series, ms1_df: pd.DataFrame, ms2_df: pd.DataFrame, scan_size=5, neighboring=True):
    related_ms1 = get_ms1_from_precursor(precursor, ms1_df)
    related_ms2 = get_ms2_from_precursor(precursor, ms2_df)
    ...
    rt_peak = (precursor['StartRT'] + precursor['EndRT']) / 2
