...
def sequence_generation_inference(cfg: DictConfig, spec_header, test_dl, model, model_gnova, device):
    ...
    fieldnames = ['graph_idx', 'pred_seq', 'pred_prob', 'pred_path']
    ...
    for d in tqdm(test_dl, total=len(test_dl)):
        ...
        try:
            precursor_moverz, precursor_charge, edge_known_list, path_mass = gen_infer.read_spec_data(idx)
