...
class Residual_seq():
    ...
    def __init__(self, seqs):
        ...
        self.aa_bins = [get_bin_by_mz(self.__aa_residual_composition[i].mass) for i in seq]
        self.step_bins = np.cumsum(self.aa_bins)
        ...
        self.step_mass = [get_mz_by_bin(i) for i in self.step_bins]
        self.mass = get_mz_by_bin(self.step_bins[-1])
...
class Ion():
    ...
    @classmethod
    def precursormass2ion(cls, precursor_mass, charge):
        return (precursor_mass + Composition('H2O').mass + charge * Composition('proton').mass) / charge
