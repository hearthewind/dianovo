...
class GNovaEncoder(nn.Module):
    def __init__(self, cfg, output_dtype):
        ...
        """
        super().__init__()
        # self.peak_feature_proj = nn.Linear(5 * 8, cfg.encoder.hidden_size)
        # self.peak_xgram_proj = nn.Linear(5, cfg.encoder.hidden_size)
        ...
        self.peak_feature_ln = nn.LayerNorm(cfg.encoder.hidden_size)

        self.head_size = cfg.encoder.d_relation // cfg.encoder.num_heads
        ...
        self.peak_flag_embedding = nn.Embedding(cfg.data.precursor_max_charge + 3, cfg.encoder.hidden_size, padding_idx=0)
        
        self.genova_encoder_layers = nn.ModuleList([ \
            GNovaEncoderLayer(hidden_size = cfg.encoder.hidden_size, num_heads=cfg.encoder.num_heads, d_relation = cfg.encoder.d_relation,
                              alpha = (2*cfg.encoder.num_layers)**0.25, beta = (8*cfg.encoder.num_layers)**-0.25,
                              dropout_rate = cfg.encoder.dropout_rate) for _ in range(cfg.encoder.num_layers)])
